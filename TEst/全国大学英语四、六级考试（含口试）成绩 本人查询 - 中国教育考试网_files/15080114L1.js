...
function furl()
{
  var url=$("link[rel=canonical]").attr("href"),j=0;
  ...
  var NAME=["www"
    ...
    ,"cet","ncre","mhk"
    //海外考试
    ,"toefl-main","ielts-main","testdaf-main","jlpt-main","topik-main","gre-main","gmat-main","bec","delf-dalf","dele","lsat","gelpe-bras","cyle","cael"
  ];
  if(location.hostname.indexOf(".neea.cn")+1)
  {
    for(var i=0;i<NAME.length;i++)url=url.replace("/"+NAME[i]+".neea.edu.cn","/"+NAME[i]+".neea.cn");
  }else
  {
    for(var i=0;i<NAME.length;i++)url=url.replace("/"+NAME[i]+".neea.cn","/"+NAME[i]+".neea.edu.cn");
  }

  if(url&&location.hostname.indexOf("cms."))
    j=url.indexOf("/",8);
  else
  ...
  if(navigator.userAgent.indexOf(" Mobile")>0)//转回手机版
  {
    if(url.indexOf("/html1")==j)
    {
      url=url.replace("/html","/xhtml");
      $.ajax({url:url.substr(j)+"?test",async:false,success:function(d)
      {
  ...
  {
    if(url==null||url=="null")return;
    if(j&&url.indexOf(location.hostname)==-1&&/^0|200511151|210519446/.test(node.father))location.replace(url);
  }
}
furl();
